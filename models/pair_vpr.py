...
from utils.blocks import DecoderBlock
from utils.modeltools import RandomMask
...
class PairVPRNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.encoder = backbones.PAIRDINOv2(**cfg.encoder)
...
class PairVPR(VPRModel):
    def __init__(self,
                config ):
        self.cfg = config
        self.device = 'cpu'
        self.image_size = (config.augmentation.img_res,config.augmentation.img_res)
        ...
        self.model =  PairVPRNet(self.cfg)
        
    

    def load_model_state_dict(self,path_to_weight: str):
        state_dict = torch.load(path_to_weight,map_location='cpu') 
        interpolate_pos_embed(self.cfg, self.model, state_dict)
    ...
    def get_descriptors(self,dataloader: DataLoader) -> torch.Tensor:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, 'Calculating descritptors...'):
                imgs, labels = batch
                map_,output = self.model(imgs.to(self.device),None,mode='global')
    ...
    def get_embedding(self, img: Image) -> List[float]:
        ...
        with torch.no_grad():
            map_, out =  self.model(img.to(self.device),None,mode='global')
            return out.detach().cpu().tolist()[0]

if __name__ == '__main__':
    cfg = OmegaConf.load('models_config/pair-vpr/speed.yaml')
    mm = PairVPR(cfg)
    mm.load_model_state_dict('saved_models/pair-vpr/pairvpr-vitB.pth')
