...
class MIXMain(pl.LightningModule):
...
class MIXMainVPR(VPRModel):
    def __init__(self,
        ...
        self.model =  MIXMain(backbone_arch = backbone_arch,pretrained = pretrained,
                          layers_to_freeze = layers_to_freeze, layers_to_crop = layers_to_crop,
                        agg_arch = agg_arch, agg_config = agg_config)
    ...
    def get_descriptors(self,dataloader: DataLoader) -> torch.Tensor:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, 'Calculating descritptors...'):
                imgs, labels = batch
                output= self.model(imgs.to(self.device)).cpu()
    ...
    def get_embedding(self, img: Image) -> List[float]:
        ...
        with torch.no_grad():
            return self.model(img.to(self.device)).cpu().tolist()[0]

if __name__ == '__main__':
    mm = MIXMainVPR(backbone_arch='resnet50', 
                layers_to_crop=[4],
                 #---- Aggregator
                agg_arch='MixVPR',
                agg_config={
                ...
                pretrained=True,
                layers_to_freeze=1)
    mm.load_model_state_dict('/media/sunveil/Data/header_detection/poddubnyy/postgraduate/VPR/gsv-cities/resnet50_MixVPR_4096_channels(1024)_rows(4).ckpt')
